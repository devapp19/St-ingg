function calculate() {
    ...
    var info = Number(document.getElementById('info').value);
      ...
      var fds = Number(document.getElementById('fds').value);
        ...
        document.getElementById("total").innerHTML = ((4*fph)+(4*fchime)+(2*info)+(2*falgebra)+(3*fcal)+(2*fds)+eng+eth)/19;

        var total = ((4*fph)+(4*fchime)+(2*info)+(2*falgebra)+(3*fcal)+(2*fds)+eng+eth)/19;
